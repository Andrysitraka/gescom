...
public function getEvaluation(){
    $sql="SELECT `idev` FROM `note_evaluation` WHERE 1 ORDER BY `idev` DESC LIMIT 1 ";
...
public function ca_mois($matricule,$mois){//CA DU COMMERCIAL SANS VENTE ACCOMPAGNEMENT
   
    //$sql_vente_mois="SELECT `idPrix`,`quantite`,`lieu`,`date`,`codeproduit` FROM `vente` WHERE `idVP` LIKE ? AND MONTH(`date`) LIKE ?";
    
    $sql_vente_mois=$sql_vente_jour="SELECT vente.idPrix ,vente.quantite FROM `vente`,historique_personnel WHERE `idVP` LIKE ? AND id_historique_coach = historique_personnel.id AND historique_personnel.activite != ? AND MONTH(`vente.date`) LIKE ?";
    ...
    //$sql_prix_mois="SELECT `prixdetail` FROM `prix` WHERE `idMission` LIKE ? AND `idProduit` LIKE ?";
     $sql_prix_mois="SELECT `prixdetail` FROM `prix` WHERE `idPrix` = ?";
...
public function ca_jour($nom_equipe,$date){//CA DU COMMERCIAL SANS VENTE ACCOMPAGNEMENT
    ...
    foreach($membre as $membre){
        //$sql_vente_jour="SELECT `idPrix`,`quantite`,`lieu`,`codeproduit` FROM `vente` WHERE `idVP` LIKE ? and `date` LIKE ?";
        //$vente=$this->queryAll($sql_vente_jour,array($membre['mtrP'],$date));
        $sql_vente_jour="SELECT vente.idPrix ,vente.quantite FROM `vente`,historique_personnel WHERE `idVP` LIKE ? AND id_historique_coach = historique_personnel.id AND historique_personnel.activite != ? AND vente.date LIKE ?";
        ...
        foreach($vente as $vente){
            ...
            $sql_prix_jour="SELECT `prixdetail` FROM `prix` WHERE `idPrix` = ?";
...
public function ca_jour_com($matricule,$date){//CA DU COMMERCIAL SANS VENTE ACCOMPAGNEMENT
    //$sql_vente_jour="SELECT `idPrix`,`quantite`,`lieu`,`date`,`codeproduit` FROM `vente` WHERE `idVP` LIKE ? AND `date` LIKE ?";
    $sql_vente_jour="SELECT vente.idPrix ,vente.quantite FROM `vente`,historique_personnel WHERE `idVP` LIKE ? AND id_historique_coach = historique_personnel.id AND historique_personnel.activite != ? AND vente.date LIKE ?";
...
//CA DU COACH AVEC VENTE ACCOMPAGNEMENT
public function ca_jour_coach($matricule,$date){
    $sql_vente_jour="SELECT `idPrix`,`quantite`,`lieu`,`date`,`codeproduit` FROM `vente` WHERE `idVP` LIKE ? AND `date` LIKE ?";
...
//CA SEULEMENT POUR LE VENTE ACCOMPAGNEMENT
public function ca_accompagnement($matricule,$date){
    $sql="SELECT `idPrix`,`quantite` FROM `vente`,historique_personnel WHERE `idVP` LIKE ? AND vente.date LIKE ? AND historique_personnel.id = vente.id_historique_coach AND historique_personnel.activite LIKE ?";
    $vente=$this->queryAll($sql,array($matricule,$date,'vente d\'accompagnement'));
    $prix_quantite=0;
...
public function upDateRapport($parametre=array()){
    $sql="UPDATE `vente` SET `id_rcc`= ? WHERE `idVente` LIKE ?";
    $query=$this->con->prepare($sql);
    $query->execute($parametre);   
}
//CA JOUR DU COM SANS VENTE ACCOMP AVEC HEURE
public function ca_du_com_heure($matricule,$date,$h1,$h2){
    $sql="SELECT vente.idPrix ,vente.quantite FROM `vente`,historique_personnel WHERE `idVP` LIKE ? AND id_historique_coach = historique_personnel.id AND historique_personnel.activite != ? AND vente.date LIKE ? AND vente.heure BETWEEN ? AND ?";
    $vente=$this->queryAll($sql,array($matricule,'vente d\'accompagnement',$date,$h1,$h2));
    $prix_quantite=0;
