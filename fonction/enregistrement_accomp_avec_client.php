...
if($uploadOk == 0){
   echo 0;
      ...
      $sql1="SELECT `designationEquipe` FROM `planing_equipe` WHERE `mtrP` LIKE ? AND `statut` LIKE 'Active'";
        ...
        $main->query($sql,array(null,$_GET['note_presentation'],$_GET['note_argumentaire'],$_GET['note_cible'],$_GET['note_comportement']));

        $idev=$main->getEvaluation(); //prend l'id de l'évaluation
                    
        foreach($content_produit as $key=>$content_produit){
              $sql="INSERT INTO `vente`(`idVente`,`codeClient`,`quantite`, `lieu`, `date`, `idVP`, `heure`,`codeproduit`, `ville`, `quartier`,`id_equipe`,`id_historique_coach`,`facture`,`idPrix`,`id_evaluation`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
              $main->query($sql,array(null,$codeClient,$content_quantite[$key],$_SESSION['idMission'],$dateDB,$commercial,$date,$content_produit,$_GET['ville'],$_GET['quartier'],$equipe['designationEquipe'],$id_coach, $idfacture,$idPrix[$key],$idev[0]));
        }
