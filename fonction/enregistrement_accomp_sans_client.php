...
if (isset($_POST['content_produit']) AND isset($_POST['content_quantite']) AND isset($_POST['ville']) AND isset($_POST['quartier']) AND isset($_POST['note_presentation']) AND isset($_POST['note_argumentaire']) AND isset($_POST['note_cible']) AND isset($_POST['note_comportement'])) {
    if (!empty($_POST['content_produit']) AND !empty($_POST['content_quantite']) AND !empty($_POST['ville']) AND !empty($_POST['quartier']) AND !empty($_POST['note_presentation']) AND !empty($_POST['note_argumentaire']) AND !empty($_POST['note_cible']) AND !empty($_POST['note_comportement'])) {
      ...
      $idPrix=json_decode($_POST['idPrix']);
        foreach ($content_produit as $key=>$content_produit) {
            ...
            $main->query($sql,array(null,null,$content_quantite[$key],$_SESSION['idMission'],date("Y-m-d"),$commercial,$date,$content_produit,$_POST['ville'],$_POST['quartier'],$equipe['designationEquipe'],$id_coach,$idPrix[$key],$idev[0]));
          }
          //var_dump($idPrix);
 }
}
