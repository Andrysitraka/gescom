 ...
 class main_function extends main
 {
   ...
   private $db_gestion_de_vente;
    function __construct(){
        $this->main=new main();
        $this->db_gestion_de_vente=new main('localhost','inexped1_gestiondevente','root','');
    }
...
public function moyenne_evaluation($com,$date_vt){
    $sql_ev="SELECT `id_evaluation` FROM `vente` WHERE `idVP` LIKE ? AND `date` LIKE ?"; 
    $evaluation=$this->main->queryAll($sql_ev,array($com,$date_vt));
    $nbr=0;$note=0;$moyenne=0;
    if($evaluation){
    ...
    $evaluation=$this->main->queryAll($sql_ev,array($com,$date_vt));
    foreach($evaluation as $evaluation){
        $sql="SELECT `presentation`,`argumentaire`,`cible`,`comportement` FROM `note_evaluation` WHERE `idev` LIKE ?";
        $notes=$this->main->query($sql,array($evaluation['id_evaluation']));
        $note=$notes['presentation']+$notes['argumentaire']+$notes['cible']+$notes['comportement'];
        $moyenne+=$note;
    }
    return $moyenne/($nbr*4);
}
