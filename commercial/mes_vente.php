    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <div class="col-lg-12">
          ...
          $sql="SELECT `prixdetail`,`prixgros` FROM `prix` WHERE `idMission` LIKE ? AND `idProduit` LIKE ? AND `statut` LIKE 'on'"; 
        ...
        <button type="button" class="btn btn-danger" data-dismiss="modal">Fermer</button>
                                   ...
                                   <td class="text-center" style="font-size:10px;"><?php
                      $sql='SELECT * FROM `prix` WHERE `idProduit` LIKE "'.$resultat['codeproduit'].'" AND `idMission`='.$resultat['lieu'].' AND `statut` LIKE "on"';
                      $prix = $main->query($sql);
                      if ($prix) {
