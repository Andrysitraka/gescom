...
$('.input-quartier').autocomplete({
    source:function(requete, reponse){
    var ville=$('.input-ville').val();
    $.get('../fonction/retour_quartier.php',{term:requete.term,ville:ville},function(data){
        if(!data.length){ 
            var result = [{
...
$('.input-ville').autocomplete({
    source:'../fonction/retour_ville.php',
    minLength: 0,
    selectFirst:true,
    minChars: 0,
    disabled: false
...
//couleur du panier au clic
   ...
   $('.status_actif').on('click',function(event){
    ...
    $('#tabcontent1').collapse('show');
    $('.privilege').on('click',function(event){
        event.preventDefault();
         var idprivilege=$(this).attr('id');
            
        if("privilege1"==idprivilege){
            $('#tabcontent2').collapse('hide');
            $('#tabcontent1').collapse('show');
            $('#privilege2').attr('style','');
            $('#privilege1').attr('style','border-bottom:1px solid;padding: 10px;box-shadow: 5px 10px #888888;');
           

        }else{
            $('#tabcontent1').collapse('hide');
            $('#tabcontent2').collapse('show');
            $('#privilege1').attr('style','');
            $('#privilege2').attr('style','border-bottom:1px solid;padding: 10px;box-shadow: 5px 10px #888888;');
