...
$('.carte').on('focusout',function(){
        ...
        if(province!=='' && ville!=='' && quartier!=='' ){
            alert ('Vide');
            
        }else{
            
           
            cart(province,ville,quartier); 
             location.reload();
        }
});
/*
...
function cart(province,ville,quartier){
var html='<div style="overflow:hidden;width: 350px;height: 400px;"><iframe width="350" height="400" src="https://maps.google.com/maps?width=350&amp;height=400&amp;hl=en&amp;q=%20%09'+province+'%20%2C%20'+quartier+'%20%2C'+ville+'%2Cmadagascar+(Titre)&amp;ie=UTF8&amp;t=k&amp;z=17&amp;iwloc=B&amp;output=embed" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe><div><small><a href="https://embedgooglemaps.com/en/">embedgooglemaps FR</a></small></div><div><small><a href="https://newyorkhoponhopoffbus.nl">new york city freestyle pass: hop-on, hop-off tour en ferry</a></small></div><style>#gmap_canvas img{max-width:none!important;background:none!important}</style></div><br />';        
...
var  date_deb=$('.date_deb').val(), date_fin=$('.date_fin').val(), idMission=$('.idMission').val(),date=$('.date').val(),ville=$('.ville').val(), province=$('.province').val(),quartier=$('.quartier').val(),IdEquipe=$('.IdEquipe').val(),Panier=$('.Panier').val();
if(date=="" || ville=="" || province==""|| quartier==""){
    $("#monModal").modal('show');
}else{
 $.post('../fonction/fonctiontPlaning.php',{date_deb:date_deb,date_fin:date_fin,idMission:idMission,date:date,ville:ville,province:province,quartier:quartier,IdEquipe:IdEquipe,Panier:Panier},function(data){
 tabledataPlaning();
 if(data.error=='true'){
     alert("Erreur")
 }else{
...
//AJOUTER PHOTO CARTE
 ...
 $('.AjoutphotoCarte').on('click',function(e){
     e.preventDefault();
  ...
  $.ajax({
            ...
            url: '../fonction/enregistrement_photoCarte_planing.php?quartier='+quartier,
            type: 'post',
            data: fd,
            contentType: false,
            processData: false,
            success: function(response){
                if(response != 0){
                  $('.titre').empty().append("Information"); 
                  $('.modal-header').empty().append("<span class='text-success text-center'><i class='fa fa-check' aria-hidden='true'></i>Insertion image reussie !</span>");
...
//DEDUCTION ET PRIME COACH AVEC SELECT DU CONTROLEUR
...
//FIN COACH CONTROLEUR
$('.detail_prix').on('click',function(){
  // e.preventDefault();
   var produit=$(this).parent().parent().find('.nom_produit').html();
   $('#myModal .modal-title').empty().append('Chargement...');
   $('#myModal .modal-body').addClass('text-center').empty().append('<div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div>');
   $('#myModal .modal-footer').remove();
   $("#myModal").modal('show');
    $.post('../fonction/retourPrixProduit.php',{produit:produit},function(data){
       $('#myModal .modal-title').empty().append('Les prix du produit '+produit+' pour chaque mission');
