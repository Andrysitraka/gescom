$(document).ready(function(){
   ...
   // heure
    function heure_coach(){
        ...
        }else if(heure<17 && heure>=13){
            $('.num_rapport').empty().append('Rapport numéro 2');
        }else{
            $('.num_rapport').empty().append('Pas de rapport');
        }
        //console.log(heure+':'+minute);
        if(((heure>=11) && (heure<13))){ // RAPPORT
            $('.lock_no').empty().append('non');
            if(!$('.lock_no').hasClass('lock_after')){
                $('.lock_no').addClass('lock_after');
                /*$('.lock_no').addClass('no_ca');*/
            }
            
            $('.lock_no').each(function(){
               if($(this).parent().parent().find('.existe_ok').hasClass('existe_ok')){
                    $(this).parent().parent().find('.colsp').next().addClass('existe');
                    $(this).empty().append('<i class="fa fa-lock" aria-hidden="true"></i>');
                }
            });
            $('.lock').empty().append('<i class="fa fa-lock" aria-hidden="true"></i>');
            
            
            
            if($('.lock_no').parent().parent().find('.colsp').next().hasClass('d-none')){
                $('.lock_no').parent().parent().find('.d-none').remove();
               //$('.existe').removeClass('existe');
            }
            if(!$('.lock').parent().parent().find('.colsp').next().hasClass('existe')){
               $('.lock').parent().parent().find('.colsp').after('<td class="text-center existe d-none" style="font-size:11px;"></td>');
            }
            
        }else if(heure>=16 && heure<17){
            $('.lock_no').empty().append('non');
            if(!$('.lock_no').hasClass('lock_after')){
                $('.lock_no').addClass('lock_after');
                /*$('.lock_no').addClass('no_ca');*/
            ...
            if(!$('.lock_after').hasClass('lock_no')){
                $('.lock_after').addClass('lock_no').removeClass('lock_after');
            }
            $('.lock_no').empty().append('<i class="fa fa-lock" aria-hidden="true"></i>');
           /* $('.lock_no').removeClass('no_ca');*/
           if(!$('.lock_no').parent().parent().find('.colsp').next().hasClass('existe')){
               $('.lock_no').parent().parent().find('.colsp').after('<td class="text-center existe d-none" style="font-size:11px;"></td>');
           }
        ...
        $('.ora').empty().append(h+heure+':'+min+minute+':'+sec+seconde);
...
*/
    ...
    $('.save_accomp').on('click',function(e){
        e.preventDefault();
            ...
            var note_presentation=$('.note_presentation').val();
            var note_argumentaire=$('.note_argumentaire').val();
            var note_cible=$('.note_cible').val();
            var note_comportement=$('.note_comportement').val();
            ...
            if(ville=="" || quartier=="" || typeof(contPro)=='undefined' || commercial=="Mes accompagnement" || evaluation=="Evaluation"){
             ...
             $('#exampleModalCenter .modal-title').empty().append('<span style="font-size:15px">Information.</span>');
              ...
              var fd="test";
                ...
                $('#exampleModalCenter').modal('show');
            $.ajax({
                    url: '../fonction/enregistrement_accomp_avec_client.php?idPrix='+idPrix+'&com='+commercial+'&note_presentation='+note_presentation+'&note_argumentaire='+note_argumentaire+'&note_cible='+note_cible+'&note_comportement='+note_comportement+'&anne='+anne+'&mois='+mois+'&jour='+jour+'&quartier='+quartier+'&ville='+ville+'&nom='+Nom+'&Prenom='+Prenom+'&sexe='+sexe+'&contact='+contact+'&content_quantite='+content_quantite+'&content_produit='+content_produit,
                    type: 'post',
                    data: fd,
                    contentType: false,
                    processData: false,
                   ...
                   $('.note').each(function(){
                    ...
                    $('#exampleModalCenter .modal-footer .btn').remove();
                    $('#exampleModalCenter').modal('show');
                     $.post('../fonction/enregistrement_accomp_client_exist.php',{idPrix:idPrix,note_presentation:note_presentation,note_argumentaire:note_argumentaire,note_cible:note_cible,note_comportement:note_comportement,com:commercial,evaluation:evaluation,content_produit:content_produit,content_quantite:content_quantite,ville:ville,quartier:quartier,codeClient:idClient[0]},function(data){
              ...
              /* $('.modal-body').empty().append('<div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div>');
                ...
                $('.modal').modal('show');*/
                 $.post('../fonction/enregistrement_accomp_sans_client.php',{idPrix:idPrix,note_presentation:note_presentation,note_argumentaire:note_argumentaire,note_cible:note_cible,note_comportement:note_comportement,com:commercial,content_produit:content_produit,content_quantite:content_quantite,ville:ville,quartier:quartier},function(data){
...
$('.save_rapport').on('click',function(e){
    ...
    var tab;    
        ...
        if((tab[0]>=11) && (tab[0]<13)){ //MIDI
            $('#exampleModalCenter .modal-title').empty().append("Chargement...");
            $('#exampleModalCenter .modal-body').addClass('text-center').empty().append('<div class="spinner-border text-primary" role="status"><span class="sr-only">Loading...</span></div>');
            $('#exampleModalCenter .modal-footer').remove();
            $('#exampleModalCenter').modal('show');
            $.post("../../fonction/ajoutRapport.php",{heure:tab[0]+':'+tab[1]+':00',description:description},function(data){
        ...
        }else if(tab[0]>=16 && tab[0]<=17){//SOIR
            $('#exampleModalCenter .modal-title').empty().append("Chargement...");
            $('#exampleModalCenter .modal-body').addClass('text-center').empty().append('<div class="spinner-border text-primary" role="status"><span class="sr-only">Loading...</span></div>');
            $('#exampleModalCenter .modal-footer').remove();
            $('#exampleModalCenter').modal('show');
            ...
            $('#exampleModalCenter .modal-header').remove();
            $('#exampleModalCenter .modal-body').empty().append('<span class="text-danger">Le rapport</span> doit etre redigé à 11h30-12h30 ou à 16h-17h  &nbsp;&nbsp;<i class="fa fa-exclamation-triangle text-danger" aria-hidden="true"></i>');
            $('#exampleModalCenter .modal-footer').remove();
            $('#exampleModalCenter').modal('show');
        }
});

$('.no_ca').on('click',function(e){
    e.preventDefault();

    if(!$(this).parent().parent().find('.colsp').next().hasClass('existe')){
         $(this).parent().parent().find('.colsp').attr('colspan','0');
         var ca = $(this).parent().parent().find('.existe_ok').html();
         var c='';
         $(this).parent().parent().find('.existe_ok').remove();
        $(this).parent().find('span').empty().append(ca);//test true si le coach n'a pas encore de ca rapport
         if(ca===undefined || ca=='Pas de vente'){
             ca='';
         }else{
             c = ca.split(",");
                ca=c[0];
      ...
      $(this).parent().parent().find('.colsp').after('<td class="text-center existe tmp" style="font-size:11px;"><input type="text" value="'+ca+'" class="cacoach1" style="width:50px"></td>');  
    } 
});
$('.ok_ca').on('click',function(e){
   e.preventDefault();
  
   var heure=$('.ora').text();
   var moi = $(this);
   tab=heure.split(':');
   if(!moi.parent().parent().find('.colsp').next().hasClass('existe_ok')){
                                    
                                   moi.parent().parent().find('.colsp').attr('colspan','');
                                   var ca = moi.parent().parent().find('.existe').find('.cacoach1').val();
                                   var commercial = moi.parent().parent().find('.com').text();
                                   
                                   var totalcacom=0,totalcacoach=0,tab;
                                   if(ca!='' && ca!==undefined && $.isNumeric(ca)){
                                       bootbox.confirm({
                                          size: "small",
                                          message: "<p>Etes-vous sûre d'enregistrer ce rapport ?</p>",
                                          locale: 'fr',
                                          callback: function(result){
                                             ...
                                             /* $('.lock_no').empty().append('<i class="fa fa-lock" aria-hidden="true"></i>');
                                              $('.lock_no').removeClass('no_ca');*/
                                              if(result){
                                                moi.parent().parent().find('.existe').remove();
                                       moi.parent().parent().find('.colsp').after('<td class="text-center existe_ok" style="font-size:11px;"><div class="spinner-border" role="status" style="width:20px;height:20px"><span class="sr-only">Loading...</span></div></td>');
                                       $.post("../../fonction/ajoutRapportCaCoach.php",{ca:ca,commercial:commercial,heure:tab[0]+':'+tab[1]+':00'},function(data){
                                           moi.parent().parent().find('.existe_ok').remove();
                                           moi.parent().parent().find('.colsp').after('<td class="text-center existe_ok existe ca_coach" style="font-size:11px;">'+data+'</td>');  
                                          /* $('.ca_coach').each(function(){
                                               tab=$(this).text().split(',');
                                               tab=tab[0].split(' ');
                                               totalcacoach+=Number(tab[0]+tab[1]);
                                            });
                                            $('.totalcacoach').empty().append(totalcacoach);*/
                                            moi.parent().find('.lock_after').removeClass('no_ca');
                                            moi.parent().find('.lock_after').removeClass('lock_no');
                                            moi.parent().find('.lock_after').empty().append('<i class="fa fa-lock" aria-hidden="true"></i>');
                                       ...
                                       //moi.parent().parent().find('.tmp').remove();
                                       if(moi.parent().find('span').html()=='' || moi.parent().find('span').html()==='Pas de vente'){ //pas de ca rapport et il a cliqué sur vide
                                          if(!moi.parent().parent().find('.existe_ok').hasClass('existe_ok')){
                                              moi.parent().parent().find('.colsp').attr('colspan','2');
                                              moi.parent().parent().find('.existe_ok').remove();
                                          }
                                           ...
                                           moi.parent().parent().find('.existe').remove();
                                           moi.parent().parent().find('.colsp').attr('colspan','2');
                                           //moi.parent().parent().find('.colsp').after('<td class="text-center existe_ok ca_coach" style="font-size:11px;">'+moi.parent().find('span').html()+'</td>');
